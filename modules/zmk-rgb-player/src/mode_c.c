 ...
 * `mode_c_is_active()` guards only stop the combo's ACTION; the capture happens two
 * layers below them, so mid-song chords that pair up (a score legitimately asks for
 * Y and P together) were swallowed: the strip flashed green, the game heard nothing.
 *
