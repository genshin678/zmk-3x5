...
static ssize_t on_control_write(struct bt_conn *conn,
    ...
    switch (cmd) {
        ...
        /* --- Light control (manual RGB path) --- */
        case LIGHT_CTRL_SET_EFFECT:
            if (len < 2) return BT_GATT_ERR(BT_ATT_ERR_INVALID_ATTRIBUTE_LEN);
            if (p[1] >= RGB_EFFECT_COUNT) {
                /* Out of range for this build - ignore rather than store a
                 * value no effect table entry can render. */
                LOG_WRN("light effect out of range: %u", (unsigned)p[1]);
                break;
            }
            if (effects_is_player_active()) {
                /* Player owns the strip: park the value, it is applied by
                 * effects_player_exit() when playback stops. */
                effects_set_active_deferred((rgb_effect_t)p[1]);
            } else {
                rgb_control_set_effect((rgb_effect_t)p[1]);
            }
            LOG_INF("light effect -> %u", (unsigned)p[1]);
            break;

        case LIGHT_CTRL_SET_BRIGHTNESS:
            if (len < 2) return BT_GATT_ERR(BT_ATT_ERR_INVALID_ATTRIBUTE_LEN);
            /* Clamped to RGB_MAX_BRIGHTNESS (96) inside - see rgb_control.c. */
            rgb_control_set_brightness(p[1]);
            break;

        case LIGHT_CTRL_SET_HUE:
            if (len < 3) return BT_GATT_ERR(BT_ATT_ERR_INVALID_ATTRIBUTE_LEN);
            rgb_control_set_hue((uint16_t)(p[1] | ((uint16_t)p[2] << 8)));
            break;

        case LIGHT_CTRL_NEXT_EFFECT:
            if (effects_is_player_active()) {
                effects_set_active_deferred(
                    (rgb_effect_t)((effects_get_active() + 1) % RGB_EFFECT_COUNT));
            } else {
                rgb_control_next_effect();
            }
            break;

        case LIGHT_CTRL_SAVE:
            rgb_control_save();
            LOG_INF("light cfg saved (persisted)");
            break;
...
/* --- BE07: Light state read (effect, brightness, hue u16 LE) --- */
static uint8_t light_state_buf[4];
static ssize_t on_light_read(struct bt_conn *conn,
                             const struct bt_gatt_attr *attr,
                             void *buf, uint16_t len, uint16_t offset) {
    uint16_t h = rgb_control_get_hue();
    light_state_buf[0] = (uint8_t)rgb_control_get_effect();
    light_state_buf[1] = rgb_control_get_brightness();
    light_state_buf[2] = (uint8_t)(h & 0xFF);
    light_state_buf[3] = (uint8_t)(h >> 8);
    return bt_gatt_attr_read(conn, attr, buf, len, offset,
                             light_state_buf, sizeof(light_state_buf));
}
...
BT_GATT_SERVICE_DEFINE(zmk_player_svc,
    ...
    /* Light state (READ) - effect, brightness, hue (u16 LE) */
    BT_GATT_CHARACTERISTIC(ZMK_PLAYER_CHRC_LIGHT,
        BT_GATT_CHRC_READ, BT_GATT_PERM_READ, on_light_read, NULL, NULL),
);
