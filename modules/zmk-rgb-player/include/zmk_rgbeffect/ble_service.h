...
#define ZMK_PLAYER_CHRC_KEYMAP     BT_UUID_DECLARE_16(0xBE06)  /* READ 15-byte user keymap */
#define ZMK_PLAYER_CHRC_LIGHT      BT_UUID_DECLARE_16(0xBE07)  /* READ 4-byte light state */
...
/* User key remap (manual path, see user_keymap.c).
 *   USER_KEYMAP_SET 0x40  + 15 bytes: HID usage ID per physical position 0..14.
 *                           Persisted to NVS; applies immediately. */
#define USER_KEYMAP_SET 0x40

/* Light control (manual RGB path, see rgb_control.c / effects.c).
 *
 * Brightness is clamped to RGB_MAX_BRIGHTNESS (96) inside
 * rgb_control_set_brightness() - that is a power ceiling for the 15-pixel
 * strip, not a bug, so a peer may send 0..255 and the value is bounded anyway.
 *
 * Changing the effect while the score player owns the strip does NOT start the
 * render tick: the value is parked and applied by effects_player_exit(), so a
 * mid-playback change takes effect when playback stops. */
#define LIGHT_CTRL_SET_EFFECT     0x50  /* + u8 effect (0..6, see rgb_effect_t) */
#define LIGHT_CTRL_SET_BRIGHTNESS 0x51  /* + u8 brightness (0..96 usable)      */
#define LIGHT_CTRL_SET_HUE        0x52  /* + u16 hue, little endian (0..359)   */
#define LIGHT_CTRL_NEXT_EFFECT    0x53  /* (no params) cycle +1                */
#define LIGHT_CTRL_SAVE           0x54  /* (no params) persist to NVS          */
